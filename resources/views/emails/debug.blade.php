...
<head>
    ...
    <title>{{ config('app.name', 'Laravel') }} - {{ __('fin-sentinel::fin-sentinel.email.header.debug') }}</title>
</head>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="max-width: {{ config('fin-sentinel.email_max_width', '90%') }}; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
                ...
                <tr>
                    <td style="background: linear-gradient(135deg, #4f46e5, #6366f1); background-color: #4f46e5; padding: 20px 24px; color: #ffffff; font-size: 18px; font-weight: bold;">
                        {{ config('app.name', 'Laravel') }} &mdash; {{ __('fin-sentinel::fin-sentinel.email.header.debug') }}
                    </td>
                </tr>
                ...
                <tr>
                    <td style="padding: 20px 24px; border-bottom: 1px solid #e5e7eb;">
                        <div style="font-size: 15px; font-weight: 600; margin-bottom: 10px; color: #4f46e5;">{{ __('fin-sentinel::fin-sentinel.email.debug.section_data') }}</div>
                        ...
                        @if($formattedData['type'] === 'model')
                            ...
                            @if(!empty($formattedData['relations']))
                                ...
                                <div style="margin-top: 12px; font-size: 12px; font-weight: 600; color: #6b7280;">{{ __('fin-sentinel::fin-sentinel.email.label.relation', ['name' => $relationName]) }}</div>
                        ...
                        @elseif($formattedData['type'] === 'collection')
                            <div style="display: inline-block; background-color: #eef2ff; color: #4f46e5; font-size: 12px; font-weight: 600; padding: 2px 10px; border-radius: 12px; margin-bottom: 8px;">{{ trans_choice('fin-sentinel::fin-sentinel.email.collection.count', $formattedData['count'], ['count' => $formattedData['count']]) }}</div>
                            ...
                            @if($formattedData['count'] > 20)
                                <div style="margin-top: 8px; font-size: 13px; color: #9ca3af; font-style: italic;">{{ __('fin-sentinel::fin-sentinel.email.collection.more', ['count' => $formattedData['count'] - 20]) }}</div>
                            @endif
                        ...
                        @elseif($formattedData['type'] === 'query')
                            ...
                            @if(!empty($formattedData['bindings']))
                                <div style="margin-top: 8px; font-size: 12px; color: #6b7280;">{{ __('fin-sentinel::fin-sentinel.email.label.bindings') }}</div>
                ...
                <tr>
                    <td style="padding: 12px 24px; background-color: #f9fafb; border-bottom: 1px solid #e5e7eb;">
                        <div style="font-size: 12px; color: #6b7280;">
                            <span style="font-weight: 600;">{{ __('fin-sentinel::fin-sentinel.email.debug.section_call_site') }}:</span>
                ...
                <tr>
                    <td style="padding: 16px 24px; border-bottom: 1px solid #e5e7eb;">
                        <div style="font-size: 13px; font-weight: 600; margin-bottom: 6px; color: #6b7280;">{{ __('fin-sentinel::fin-sentinel.email.debug.section_request') }}</div>
                        <table width="100%" cellpadding="4" cellspacing="0" style="font-size: 13px;">
                            @if(isset($requestContext['context']))
                                <tr>
                                    <td style="width: 70px; font-weight: 600; color: #6b7280;">{{ __('fin-sentinel::fin-sentinel.email.label.context') }}</td>
                                    <td>{{ $requestContext['context'] }}</td>
                                </tr>
                                <tr>
                                    <td style="width: 70px; font-weight: 600; color: #6b7280;">{{ __('fin-sentinel::fin-sentinel.email.label.command') }}</td>
                                    <td style="font-family: 'SFMono-Regular', Consolas, 'Liberation Mono', Menlo, monospace;">{{ $requestContext['command'] }}</td>
                                </tr>
                            @else
                                <tr>
                                    <td style="width: 70px; font-weight: 600; color: #6b7280;">{{ __('fin-sentinel::fin-sentinel.email.label.url') }}</td>
                                    <td style="word-break: break-all;">{{ $requestContext['url'] ?? '' }}</td>
                                </tr>
                                <tr>
                                    <td style="width: 70px; font-weight: 600; color: #6b7280;">{{ __('fin-sentinel::fin-sentinel.email.label.method') }}</td>
                                    <td>{{ $requestContext['method'] ?? '' }}</td>
                                </tr>
                                <tr>
                                    <td style="width: 70px; font-weight: 600; color: #6b7280;">{{ __('fin-sentinel::fin-sentinel.email.label.user') }}</td>
                ...
                <tr>
                    <td style="padding: 16px 24px; border-bottom: 1px solid #e5e7eb;">
                        <div style="font-size: 13px; font-weight: 600; margin-bottom: 6px; color: #6b7280;">{{ __('fin-sentinel::fin-sentinel.email.debug.section_environment') }}</div>
                        <table width="100%" cellpadding="4" cellspacing="0" style="font-size: 13px;">
                            <tr>
                                <td style="width: 110px; font-weight: 600; color: #6b7280;">{{ __('fin-sentinel::fin-sentinel.email.label.environment') }}</td>
                                <td>{{ $environmentContext['app_env'] ?? '' }}</td>
                            </tr>
                            <tr>
                                <td style="width: 110px; font-weight: 600; color: #6b7280;">{{ __('fin-sentinel::fin-sentinel.email.label.php_version') }}</td>
                                <td>{{ $environmentContext['php_version'] ?? '' }}</td>
                            </tr>
                            <tr>
                                <td style="width: 110px; font-weight: 600; color: #6b7280;">{{ __('fin-sentinel::fin-sentinel.email.label.laravel') }}</td>
                ...
                <tr>
                    <td style="padding: 14px 24px; background-color: #f3f4f6; font-size: 12px; color: #9ca3af; text-align: center;">
                        {{ __('fin-sentinel::fin-sentinel.email.footer') }} &middot; {{ $environmentContext['timestamp'] ?? '' }}
                    </td>
                </tr>
